...
gpioLED_dev_t gpioLED_dev = { .dev_status = 1 };
...
int gpioLED_open(struct inode *inode, struct file *filp) {
    ...
    // 会和中断发生死锁, 驱动拿了锁, 中断上下文又需要获得锁,但是中断不会挂起休眠, 因此在非中断函数中应该尽量使用spin_lock_irqsave
    // spin_lock(&gpioLED_dev.spinlock); 
    spin_lock_irqsave(&gpioLED_dev.spinlock, irq_reg);
    if (gpioLED_dev.dev_status == 0) {
        printk("gpioLED busy\n");
        spin_unlock_irqrestore(&gpioLED_dev.spinlock, irq_reg);
        return -EBUSY;
    }
    gpioLED_dev.dev_status = 0;
    // spin_unlock(&gpioLED_dev.spinlock);
    spin_unlock_irqrestore(&gpioLED_dev.spinlock, irq_reg);
...
static int __init gpioLED_init(void) {
     ...
     * 初始化spinlock
     */
    spin_lock_init(&gpioLED_dev.spinlock);
    /* 
     * 1 获取设备id 
     */
    gpioLED_dev.major = 0;
