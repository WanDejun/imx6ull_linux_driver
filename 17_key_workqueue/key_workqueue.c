...
typedef struct {
    ...
    void (*work_function)(struct work_struct *work);   // work回调函数
} key_worker_t;
...
static void timer0_function(unsigned long arg) {
    key_dev_t *dev = (key_dev_t*)(arg);
    int key_val = gpio_get_value(dev->irqkey[0].gpio);
    unsigned long flag;

    spin_lock_irqsave(&dev->irq_lock, flag); // 保护dev->key_status
    if (key_val == 0) {
        printk("key0 down!\n");
...
static void work_function(struct work_struct *worker) { // 中断下半部, 读取按键键值
    key_worker_t* key_worker = container_of(worker, key_worker_t, work);
    mod_timer(&key_dev.irqkey[key_worker->key_nr].timer, jiffies + msecs_to_jiffies(10));
}

static irqreturn_t key0_irq_handler(int val, void* dev) { // 中断函数
    key_dev_t* _dev = dev;
    schedule_work(&_dev->irqkey[0].key_worker.work); // work回调

    return IRQ_HANDLED;
}

ssize_t key_read(struct file *filp, char __user *buf, size_t cnt, loff_t *lof) { // 获取按键状态
    u32 err, copy_size = ((KEY_STATUS_ARRAY_NUM << 5) ? cnt : (KEY_STATUS_ARRAY_NUM << 5)); // 判断需要返回几个值
    key_dev_t* dev = (key_dev_t*)filp->private_data;
    char kernel_buf[copy_size];
    int i;

    for (i = 0; i < copy_size; i++) { // 读取键值, 生成返回值
    ...
    */
    for (i = 0; i < KEY_NUM; i++) {
        ...
        */
         ...
         * 6 timer消抖定时器初始化
         */
        dev->irqkey[i].timer_function = dev->timer_function[i];
        init_timer(&dev->irqkey[i].timer);
        dev->irqkey[i].timer.function = dev->irqkey[i].timer_function;
        dev->irqkey[i].timer.data = (unsigned long)(dev);

        /**
         * @function: 7 下半部work_queue初始化
        */
        dev->irqkey[i].key_worker.work_function = work_function; // 定义按键中断的下半部(work_queue回调,用于消抖)
        dev->irqkey[i].key_worker.key_nr = i;
        INIT_WORK(&key_dev.irqkey[i].key_worker.work, key_dev.irqkey[i].key_worker.work_function);
    }
...
void key_gpio_exit(key_dev_t* dev) {
    ...
    for (i = 0; i < KEY_NUM; i++) {
        cancel_work_sync(&dev->irqkey[i].key_worker.work); // 释放work_queue
        del_timer_sync(&dev->irqkey[i].timer);
        free_irq(dev->irqkey[i].irq_nr, dev); // 释放每一个gpio和对应的中断
        gpio_free(dev->irqkey[i].gpio); // 释放gpio
    }
}
...
static int __init key_init(void) {
    ...
    key_dev.timer_function[0] = timer0_function;
